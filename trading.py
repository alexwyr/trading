...
class Game:
    def __init__(self, num_bots=3, num_rounds=5, num_central_cards=5, minimum=0, maximum=1000,
                 sample_distribution=uniform.rvs, mean_value=uniform.mean(), easy_mode=False):
        ...
        for game_round in range(1, self.num_rounds + 1):
            ...
            for bot in self.bots:
                ...
                if sell_amount != 0:
                    ...
                    print('Bot %d sold %d shares%s' % (
                    bot.bot_id, sell_amount, (' for %d.' % (sell_amount * bid)) if self.easy_mode else '.'))
                if buy_amount != 0:
                    delta_shares -= buy_amount
                    delta_value += buy_amount * ask
                    print('Bot %s purchased %d shares%s' % (
                    bot.bot_id, buy_amount, (' for %d.' % (buy_amount * ask)) if self.easy_mode else '.'))
            ...
            if game_round < self.num_rounds:
                ...
                print('You ended with %d credits and %d shares with a score of %d' % (
                value, shares, value + shares * final_value))


def get_params():
    while True:
        try:
            num_bots, num_rounds, minimum, maximum = [int(param) for param in input(
                'Type in num bots, num rounds, minimum, maximum separated by spaces: ').strip().split()]
            break
